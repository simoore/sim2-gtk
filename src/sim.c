...
sim_change_creature_energy (Creature *creature, int d_energy)
{
	...
	{
		if (creature->type == PREY)	location->prey_breed--;
		else						location->pred_breed--;
...
sim_set_creature_invisible (Creature *creature)
{
	...
	if (creature->type == PREY)
	{
		...
		if (creature->energy > 25)	location->prey_breed--;
	}
	else
	{
		location->pred_num--;
		location->pred_resting++;
		if (creature->energy > 25)	location->pred_breed--;
	}
...
sim_move (Creature *creature)
{
	...
	if (creature->x_coord >= SIZE)	creature->x_coord = 0;
	else if (creature->x_coord < 0)	creature->x_coord = SIZE-1;
	if (creature->y_coord >= SIZE)	creature->y_coord = 0;
	else if (creature->y_coord < 0)	creature->y_coord = SIZE-1;
...
sim_act (gpointer data, gpointer user_data)
{
	...
	if (c->count_down > 0)	c->count_down--;
	if (c->state != REST)	c->awake_daily++;
	if (sim_is_creature_eaten (c) == TRUE) return;

    if (c->count_down == 0)
    {
		// ending long action
		if (c->visible == FALSE)	sim_set_creature_visible (c);
		...
		if (c->energy <= 0)
		{
			...
			if (c->type == PRED)	
				if (c->visible == TRUE) location->pred_num--;
				else					location->pred_resting--;
			else 
				if (c->visible == TRUE) location->prey_num--;
				else					location->prey_resting--;
		}
    }	
}
...
sim_get_location_status (int x, int y)
{
    ...
    if (location->prey_num > 0) return PREY_HERE;
    if (location->pred_num > 0) return PRED_HERE;
    if (location->prey_resting > 0) return PREY_RESTING_HERE;
    if (location->pred_resting > 0) return PRED_RESTING_HERE;
    return NOTHING_HERE;    
}
...
sim_check ()
{
	...
	for (i = 0; i < SIZE; i++)
        for (j = 0; j < SIZE; j++)
        {
			...
			if (location->prey_num < 0) g_print ("insane prey_num\n");
    		if (location->pred_num < 0) g_print ("insane_pred_num\n");
			if (location->prey_breed < 0) {g_print ("insane_prey_breed\n");
			g_print ("location %d,%d --\n", i, j);}
    		if (location->pred_breed < 0) g_print ("insane_pred_breed\n");
    		if (location->prey_resting < 0) g_print ("insane_prey_resting\n");
    		if (location->pred_resting < 0) g_print ("insane_pred_resting\n");
			if (location->prey_kill < 0) g_print ("insane_prey_kill\n");

			if (location->prey_num < location->prey_breed)	g_print ("too many prey\n");
			if (location->prey_num < location->prey_breed)	g_print ("too many pred\n");
			if (location->prey_num < location->prey_kill)   g_print ("prey escaped\n");
